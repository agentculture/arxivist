...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope arxivist
...
scope:** the default personal scope is **private** (`--scope arxivist
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `arxivist`). Private records are served only to a recall in the same
