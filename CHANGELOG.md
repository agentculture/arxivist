...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `arxivist/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=arxivist` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/arxivist/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/arxivist/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: arxivist`,
