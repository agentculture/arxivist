"""``arxivist learn`` — the learnability affordance.
...
from arxivist.cli._output import emit_result

_TEXT = """\
arxivist — a clonable template for AgentCulture mesh agents.
...
Commands
--------
  arxivist whoami             Identity from culture.yaml.
  arxivist learn              This self-teaching prompt.
  arxivist explain <path>...  Markdown docs for any noun/verb path.
  arxivist overview           Descriptive snapshot of the agent.
  arxivist doctor             Check the agent-identity invariants.
  arxivist cli overview       Describe the CLI surface itself.
...
def _as_json_payload() -> dict[str, object]:
    return {
        ...
        "json_support": True,
        "explain_pointer": "arxivist explain <path>",
    }
