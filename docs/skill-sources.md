...
arxivist vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. arxivist
...
#   - consumer-identifying prose: `guildmaster` → `arxivist` (NOT
...
If a re-sync would lose a arxivist adaptation, lift the change
